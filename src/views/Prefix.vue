<template>
  <form @submit.prevent="submitHundler">
    <h2 class="mb-2 mt-2" style="text-align: center">
      Добавление префиксов к каждой строке онлайн
    </h2>
    <h3 style="text-align: center" class="mb-2 mt-2">
      Инструмент для добавления слова или символа в начало (префикс) или конец (суффикс)
    ...
    <div class="formgrid grid">
      ...
      <div class="col-2 mt-2">
        <Button label=" Очистить" class="pi pi-trash" @click="clear" />
      </div>
    </div>
  ...
  methods: {
    submitHundler() {
      ...
      for (let i = 0; i < array.length; i++) {
        if (this.prefix) array[i] = elem + array[i];
        if (this.postfix) array[i] = array[i] + elem;
      }
